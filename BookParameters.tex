...
\pgfmathsetmacro\TotalPageCount{164}% Must be manually entered
...
\def\TheMainTitle{Black Rock Dead}
\def\TheSubTitle{The intentional and accidental deaths}
\def\TheSubTitleA{In the}
\def\TheSubTitleB{Black Rock Desert, Nevada}
\def\TheBookSeries{The Black Rock Dead Series}

\def\TheAuthor{Lucius Arcularius}
\def\TheAuthorA{Lucius}
\def\TheAuthorB{Arcularius}
\def\TheAuthorLNF{Arcularius, Lucius}

\def\TheCopyrightYear{2023}
\def\TheEdition{First Edition}

\def\ThePublisherName{Publisher Name TBD - short form}% Just the short form name
\def\ThePublisherLineA{Publisher Line A}% Just one part of the name
\def\ThePublisherLineB{Publisher Line B}% Just one part of the name
\def\ThePublisher{\ThePublisherName}% Add Ltd, Inc, LLC etc here
\def\ThePublisherAddrA{Pub AddrA}
\def\ThePublisherAddrB{Pub AddrB}
\def\ThePublisherCity{Pub City}
\def\ThePublisherState{Pub State}

\def\ThePrinter{TBD}

\def\TheSubjectArea{Black Rock Desert, Nevada History, Real Crime, Nonfiction}

\def\TheKeywords{Black Rock Desert, Nevada History, Real Crime, Nonfiction}
...
\def\PrintISBN{979-8-218-04366-7}% 
\def\PrintISBNShort{8-218-04366-7}%
%\def\HardcoverISBN{978-4-4444444-4-6}%
%\def\HardcoverISBNShort{4-4444444-4-9}%  
\def\EbookISBN{979-8-218-04367-4}%
\def\EbookISBNShort{5-5555555-5-7}%
...
\def\PrintPrice{29.95}% Price in dollars and cents for barcode.
...
\def\TheLCPCN{2021xxxxxx}

%% ----------------------------------------------------------------------------
\def\TheCopyrightKeywords{1. Black Rock Desert; 2. Nevada History; 3. True Crime}

\def\ShortDescription{% Normally restricted to 300 t0 350 characters depending on publisher.
\textsf{\textbf{Black Rock Dead} -- The Black Rock Desert of northwestern Nevada has a long history and along with that history are a series of accidental and intentional deaths. FIXME: Normally restricted to 300 t0 350 characters depending on publisher. }}

\def\BackDescription{% Typically max 2000-4000 characters
\textsf{\textbf{Black Rock Dead} -- The Black Rock Desert of northwestern Nevada has a long history and along with that history are a series of accidental and intentional deaths. FIXME: Typically max 2000-4000 characters}}


\def\AuthorBio{% 
\textsf{\textbf{Lucius Arcularius} died at Wall Spring, Nevada on March 1, 1865.}}
...
\newcommand{\bf}[1]{\textbf{#1}}% Legacy \bf support.
