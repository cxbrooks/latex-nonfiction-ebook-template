...
% cxh: fixme package must be used before imakeidx to avod "! Undefined control sequence. <argument> \@indexfile"
...
\ifthenelse{\boolean{NOHYPERREF}}{
  ...
  \ifthenelse{\boolean{TOHTML}}{
    ...
    \usepackage[pdftex,
            ...
            hidelinks,colorlinks=true,linkcolor=black,citecolor=black, %https://tex.stackexchange.com/questions/609681/change-citations-color-to-blue
...
\usepackage{xspace}                     % Useful in parameterless macros. Generating HTML needs the xspace package.
...
\newcommand{\ChapterNotesName}[1]{%
  \ifthenelse{\boolean{CHAPTERNOTES}}{%
    ...
    \addtoendnotes{
      \bigskip\StopIndent
      \large\textsf{Chapterxxx \thechapter\, --- #1}
      \normalsize\normalfont\bigskip
    }
  }%    
}
...
\newcommand{\Image}[2][]{%
 \ifthenelse{\boolean{NOCOLOR}}{%
  ...
  \ifthenelse{\boolean{THUMBS}}{%
   \IfFileExists{\ImageSource/#2_tn.png}{\IMAGE[#1]{#2_tn.png}}{%
    \IfFileExists{\ImageSource/#2_tn.jpg}{\IMAGE[#1]{#2_tn.jpg}}{%
     \IfFileExists{\ImageSource/#2_tn.pdf}{\IMAGE[#1]{#2_tn.pdf}}{%
      \IfFileExists{\ImageSource/#2_tn.bmp}{\IMAGE[#1]{#2_tn.bmp}}{%
